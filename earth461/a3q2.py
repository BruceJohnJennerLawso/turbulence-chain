...
def conductivityByPlume(plumeConductivity, coilSpacing, cumulativeResponseFunction):
	...
	layerList.append(layer(40, 600000))
...
if(__name__ == "__main__"):
	...
	dx = 100.0 - 15.0
	print "Slopes:"
	print "a %f" % ((a[len(a)-1] - a[0])/dx)
	print "b %f" % ((b[len(b)-1] - b[0])/dx)
	print "c %f" % ((c[len(c)-1] - c[0])/dx)
	print "d %f" % ((d[len(d)-1] - d[0])/dx)
	print "e %f" % ((e[len(e)-1] - e[0])/dx)
	print "f %f" % ((f[len(f)-1] - f[0])/dx)
	...
	graphLin("Apparent Conductivity by Plume conductivity", "plume2.png", "Apparent Conductivity (mS/m)", "Plume Conductivity (mS/m)", 0, 150, 0, 65, True, datasetA, datasetB, datasetC, datasetD, datasetE, datasetF)
